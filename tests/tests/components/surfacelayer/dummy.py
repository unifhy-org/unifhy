...
class Dummy(SurfaceLayerComponent):
    ...
    _inputs_info = {
        ...
        "driving_b": {"units": "1", "kind": "dynamic"},
        "driving_c": {"units": "1", "kind": "dynamic"},
        "ancillary_c": {"units": "1", "kind": "static"},
    }
    # _parameters_info = {}
    # _constants_info = {}
    _states_info = {
    ...
    def run(
        ...
        **kwargs
        ...
        output_x, _ = self.spacedomain.route(
            driving_a + driving_b + driving_c + transfer_n - state_a.get_timestep(0)
        )

        return (
            # to exchanger
            {
                "transfer_i": driving_a
                + driving_b
                + transfer_l
                + ancillary_c * state_a.get_timestep(0),
                "transfer_j": driving_a
                + driving_b
                + driving_c
                + transfer_k
